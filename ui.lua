--func
uiEnabled = true -- set to true so it can load
local function killSwitch()
    if not uiEnabled then
        game:GetService("StarterGui"):SetCore("SendNotification", {
            Title = "[Lumina]";
            Text = "Sorry the interface is not enabled!, check again later.";
            Duration = 5;
        })
        return false
    end
...
local function identifyexecutor()
  return "Lumina", synversion 
end

-- loadstring(game:HttpGet("https://raw.githubusercontent.com/RileyDoesCode/Lumina/refs/heads/main/env.lua"))() -- env
...
getgenv().IS_LUMINA_LOADED = false
local oldr = request 
getgenv().request = function(options)
	if options.Headers then
		options.Headers["User-Agent"] = "Lumina/RobloxApp/" .. synversion
	else
		options.Headers = {["User-Agent"] = "Lumina/RobloxApp/" .. synversion}
	end
...
getgenv().require = function(scr) -- not mine
	...
	if (type(scr) == "number") then 
		if not game:GetObjects('rbxassetid://' .. scr)[1] then 
			warn("[ Lumina ]: Require failed: invalid asset ID")
			return 
		end
		if typeof(game:GetObjects('rbxassetid://' .. scr)[1]) == "Instance" and game:GetObjects('rbxassetid://' .. scr)[1].ClassName == "ModuleScript" then
			if game:GetObjects('rbxassetid://' .. scr)[1].Name == "MainModule" then 
				if game:GetObjects('rbxassetid://' .. scr)[1].Source ~= "" then 
					...
					warn("[ Lumina ]: Require failed: cant require a modulescript with no code")
				end
			else 
				warn("[ Lumina ]: Require failed: require asset id failed")
			end
		end
		return
	end
...
check("getaffiliateid", function()
    ...
    assert(getgenv().getaffiliateid() == "Lumina", "getaffiliateid function test failed")
end)
	...
	elseif drawingType == "Image" then
		...
		return setmetatable(table.create(0), {
	      ...
	      elseif index == "Data" then
		        error("Attempt to access restricted property 'Data'")
		      return nil
	       end
      end
...
function Bridge:InternalRequest(body, timeout)
	...
	if body.Url then
		...
		error("[Lumina Error]: Unknown error", 2)
		return
	end
	...
	if success and result then
		error("[Lumina Error]: " .. tostring(result), 2)
	end

	error("[Lumina Error]: Unknown server error", 2)
end
...
function Bridge:request(options)
	...
	return {
		Success = false,
		StatusMessage = "[Lumina Error]: webServer connection failed:  " .. self.serverUrl,
		StatusCode = 599;
...
if not shared.vulnsm then 
	...
	getgenv().rconsoleclear = function()
		Bridge:rconsole("cls") 
		rconsolesettitle("Lumina is NOT fat!")
	end
		...
		if type(real) == "userdata" then
			...
			meta.__index = function(s,k)
				if table.find(vulnFuncTbl, k) then 
					return function()
						error("[ Lumina ]: "..tostring(k).." isn't available.")
					end
...
print("[ LUMINA ]: Added functions to the env.")
getgenv().IS_LUMINA_LOADED = true

-- this shit function makes my script fucked
--[[pcall(function()
    local HttpService = game:GetService("HttpService")
    local response = game:HttpGet("https://weao.xyz/api/status")
